package nl.gogognome.lib.awt.layout;
...
/**
 * A vertical layout manager similar to {@link java.awt.FlowLayout}.
 * Like FlowLayout components do not expand to fill available space except when the horizontal alignment
 * is <code>BOTH</code>
 * in which case components are stretched horizontally. Unlike FlowLayout, components will not wrap
 * to form another column if there isn't enough space vertically. VerticalLayout can optionally
...
public class VerticalLayout implements LayoutManager{
	...
	private int calcTotalHeight(Container parent) {
		int height = 0;
		...
		height -= vgap; //otherwise there's a vgap too many
		return height;
	}

	private void doLayout(Container parent, int y) {
		Insets insets = parent.getInsets();
		Dimension pd=parent.getSize();
		for (int i=0; i<parent.getComponentCount(); i++){
			Component c = parent.getComponent(i);
